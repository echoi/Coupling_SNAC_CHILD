...
static inline
double vecprod(int p0,int p1,int p2,vector<point*> p){
  return
...
static inline
double orient2d(int p0,int p1,int p2,vector<point*> p){
#ifdef DONT_USE_PREDICATE
  return vecprod(p0,p1,p2,p);
#else
	//cout << p0 <<" "<< p1 <<" "<< p2 << endl;
	vector<double> tmp;
	//cout << p[p0]->x() << " ------" << p[p0]->y() << endl;
	tmp.push_back(p[p0]->x()); tmp.push_back(p[p0]->y());
	vector<double> tmp1;
	//cout << p[p1]->x() << " ------" << p[p1]->y() << endl;
	tmp1.push_back(p[p1]->x()); tmp1.push_back(p[p1]->y());
	vector<double> tmp2;
	//cout << p[p2]->x() << " ------" << p[p2]->y() << endl;
	tmp2.push_back(p[p2]->x()); tmp2.push_back(p[p2]->y());
  return predicate.orient2d(tmp, tmp1, tmp2);
#endif
}

static inline
bool needswap(int i1, int i2, int i3, int i4, vector<point*> p){
#ifdef DONT_USE_PREDICATE
...
#else
	vector<double> tmp;
	tmp.push_back(p[i4]->x()); tmp.push_back(p[i4]->y());
	vector<double> tmp1;
	tmp1.push_back(p[i2]->x()); tmp1.push_back(p[i2]->y());
	vector<double> tmp2;
	tmp2.push_back(p[i3]->x()); tmp2.push_back(p[i3]->y());
	vector<double> tmp3;
	tmp3.push_back(p[i1]->x()); tmp3.push_back(p[i1]->y());
  return predicate.incircle(tmp, tmp1, tmp2, tmp3)
    > 0 ? true:false;
#endif
}
...
#endif
void point::write(std::ofstream& f) const {f<<"x()= " << x()<<' '<<"y()= " <<y()<< '\n';}


#if defined(DEBUG_PRINT)
void edge::print(vector<point*> p) const {p[from]->print();p[to]->print();}
#endif
void edge::write(std::ofstream& f,const vector <point*>&  p) const {
  p[from]->write(f);p[to]->write(f);}
bool edge::visible(vector<point*> p,int i) const {
...
static
int tt_swap(int tint, edge e[], vector<point*> p){
...
void start_aligned_point(int &lower_hull_pos, int &upper_hull_pos,
			 ...
			 int npoints, vector<point*> p, edge *edges,
			 cyclist &hull){
...
static
void triangulate(int npoints,vector<point*> p, int *pnedges, edge** edges_ret){
...
static
void tt_verify_sort(int npoints, vector <point*> p){
...
static
void moved_duplicated_points(int npoints, vector <point*> p, int &npoints_unique){
  if (npoints <= 1) {
...
void tt_sort_triangulate(int npoints, vector<point*> p,
			 int *pnpoints_unique,
...
#if defined(TIMING)
  {
    ...
    //array p will be replaced with the array sorted in x
	 //cout << "TEST: " << p[npoints-2]->x() << " "<< p[npoints-2]->y() << endl;
    heapsort(npoints,p);
...
void tt_build_elem_table(int npoints, vector<point*> p,
