...
BuildDelaunayMeshTipper()
{
    vector<point*> p;
   {
     ...
     for( cn=nodIter.FirstP(); !(nodIter.AtEnd()); cn=nodIter.NextP()){
		//cout <<"TEST CN:" << cn->getX() << " "<< cn->getY()<< " "<< cn->getID()<< endl;
		p.push_back(new point(cn->getX(), cn->getY(), cn->getID()));
       ++inode;
     }
     if (0) { // DEBUG
   ...
   this->ntri = nelem;
   {
     ...
     std::cout << "Setting up edg pointer..." << std::flush;
     {
       ...
       vector< int > p2sp; p2sp.assign(nnodes,0);
	 ...
	 tTriangle * newtri = new tTriangle( ielem,
   ...
   SetmiNextTriID( triList.size() );

   // assertions
   assert( edgeList.getSize() == 2*nedgesl );
   assert( static_cast<int>(triList.size()) == nelem );
