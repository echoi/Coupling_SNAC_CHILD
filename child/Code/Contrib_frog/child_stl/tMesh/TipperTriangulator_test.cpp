...
static
void write_point(int npoints, vector<point*> p)
{
...
static
void write_edge(const edge *edges, vector<point*> p)
{
...
static
void test_sort_triangulate(int npoints, vector <point*> p){
  int nedges;
  edge* edges = NULL;
...
static
void generate_dataset(int n,vector <point*> p){
  //make a set of points perturbed from a uniform grid
  srand(0);
  for (int i=0;i<n;i++){
    for(int j=0;j<n;j++){
      p[i+j*n] = new point(double(i)+double(rand())/RAND_MAX*n*1.e-3,
