...
int Next3Delaunay( deque< tSubNode *> nbrList)
{
   if (0) { //DEBUG
     static int ncalls = 0;
     ncalls++;
   ...
   tSubNode *nbrnd = nbrList.front();
   vector< double > p0( nbrList[0]->get2DCoords() );
   vector< double > p1( nbrList[1]->get2DCoords() );
   vector< double > p2( nbrList[2]->get2DCoords() );
   ...
   typename deque < tSubNode * >::iterator it;
   it = nbrList.begin();
   while( ( cn  = *(it++) ) != nbrnd )
   {
     const vector< double > ptest = cn->get2DCoords();
...
MakeLayersFromInputData( const tInputFile &infile )
{
   ...
   vector<double> dgradebrhelp;  dgradebrhelp.assign(numg,0);
   double sumbr = 0.;
   i=0;
   char add[2];
   add[0]='1';
...
MeshDensification( const tInputFile &infile )
{
   ...
   if( initMeshDensLevel)
   {
     ...
     vector<double> newx, newy, newz;   // Lists of new coords
     ...
     for( j=1; j<=initMeshDensLevel; j++ )
     {
       ...
       UpdateMesh();
       nnewpoints = ntri = triList.size();  // no. of triangles in the list
       newx.assign(nnewpoints,0);
       newy.assign(nnewpoints,0);
       newz.assign(nnewpoints,0);

       // Compute and store the x,y,z coordinates of the points to be added
       ct = triList.front();     // start with the first triangle
       {
	 for( int i=0; i<ntri; i++ )    // loop through the triangles
	   {
	     assert( ct!=0 );
	     vector<double>xy = ct->ePtr(0)->getRVtx();  // get the coords
	     newx[i] = xy.at(0);
	     newy[i] = xy.at(1);

	     // Now find the z coordinate using interpolation
	     vector <double> zvals;// z values of a triangle's 3 nodes
					zvals.push_back(ct->pPtr(0)->getZ());
					zvals.push_back(ct->pPtr(1)->getZ());
					zvals.push_back(ct->pPtr(2)->getZ());
...
MakeMeshFromInputData( const tInputFile &infile )
{
...

   {
       ...
       for ( int i=0; i<ntri; i++ )
	 {
	   ...
	   tTriangle * newtri = new tTriangle( i,
   ...
   {
     deque < tTriangle * > TriTable(triList); // for fast lookup per ID
     for( int i=0; i<ntri; ++i )
       {
...
MakePointBoundary( const ParamMMFS_t &Param, const tInputFile &infile,
		   deque< tSubNode * > bndList, tRand &rand)
{
   int i,                        // counters
   ...
   switch( Param.boundType ) 
   {
	   case ParamMMFS_t::kCornerOutlet:
	   {
		   ...
		   tempnode.setBoundaryFlag( kOpenBoundary );
		   nodeList.insertAtBack( tempnode );
		   bndList.push_back( nodIter.LastP() );
	   ...
	   case ParamMMFS_t::kOppositeSidesOpen:
	   {
		   ...
		   for( i=n-1; i>0; i--, miNextNodeID++ )
		   {
			   ...
			   tempnode.setID( miNextNodeID );
			   nodeList.insertAtBoundFront( tempnode );
			   bndList.push_back( nodIter.FirstBoundaryP() );
		   }
		   tempnode.setBoundaryFlag( kClosedBoundary );
		   n = ROUND( Param.yGrid / Param.delGrid );
		   for( i=n; i>=0; i--, miNextNodeID++ )
		   ...
		   bool makeMesh, tRand &rand )
{
   ...
   double slope;
   vector< double > xyz; xyz.assign(3,0);
   tSubNode tempnode( infile );  // temporary node used to create node list
...
MakeMeshFromPoints( const tInputFile &infile )
{
   ...
   vector<double> x, y, z;          // arrays of x, y, and z coordinates
   vector<int> bnd;                 // array of boundary codes
   ...
   deque <tSubNode *> supertriptlist;
   supertriptlist.push_back( stp1 );
   supertriptlist.push_back( stp2 );
   supertriptlist.push_back( stp3 );
   //supertriptlist.makeCircular();
   MakeTriangle( supertriptlist);
...
MakeRandomPointsFromArcGrid( const tInputFile &infile )
{
   ...
   tEdge* ce;
   nodeListIter_t nI( nodeList );
   deque <tSubNode *> supertriptlist, deletelist;
   ...
   for( cn = nI.FirstBoundaryP(); !( nI.AtEnd() ); cn = nI.NextP() )
   {
      ...
      else deletelist.push_back( cn );
   }
   int size = deletelist.size();
   for (int i= 0; i< size; i++)
   {
      /* cn =*/ deletelist.pop_front();
   }
...
CheckMeshConsistency( bool boundaryCheckFlag /* default: true */)
{
   ...
   for (int j=0; j<static_cast<int>(triList.size()); j++)
   {
      for( i=0; i<=2; i++ )
      {
         // Valid point i?
         if( (cn=triList[j]->pPtr(i)) == NULL)
         {
            std::cerr << "TRIANGLE #" << triList[j]->getID()
                 << " has an invalid point " << i << std::endl;
            goto error;
         }
         // Valid edge i?
         if( (ce=triList[j]->ePtr(i)) == NULL)
         {
            std::cerr << "TRIANGLE #" << triList[j]->getID()
         ...
         if( ce->getDestinationPtrNC()!=triList[j]->pPtr((i+2)%3) )
         {
            std::cerr << "TRIANGLE #" << triList[j]->getID()
         ...
         if( (optr = triList[j]->tPtr(i)) != 0 )
         {
            nvop = optr->nVOp(triList[j]); // Num (0,1,2) of opposite vertex in optr
            if( nvop < 3 )
            {
               if( triList[j]->pPtr((i+1)%3) != optr->pPtr((nvop+2)%3)
                   || triList[j]->pPtr((i+2)%3) != optr->pPtr((nvop+1)%3) )
               {
                  std::cerr << "TRIANGLE #" << triList[j]->getID()
                       << ": opposite triangle " << i << " does not share nodes "
                       << (triList[j]->pPtr((i+1)%3))->getID() << " and "
                       << (triList[j]->pPtr((i+2)%3))->getID() << std::endl;
                  goto error;
               }
            }
            else
         ...
         {
            if( (triList[j]->pPtr((i+1)%3))->getBoundaryFlag()==kNonBoundary
                || (triList[j]->pPtr((i+2)%3))->getBoundaryFlag()==kNonBoundary )
            {
               std::cerr << "TRIANGLE #" << triList[j]->getID()
                    ...
                    << (triList[j]->pPtr((i+1)%3))->getX() << ' ' << (triList[j]->pPtr((i+1)%3))->getY() << ' '
                    << BoundName((triList[j]->pPtr((i+1)%3))->getBoundaryFlag())
                    << " and \n" << (triList[j]->pPtr((i+2)%3))->getX() <<' ' << (triList[j]->pPtr((i+2)%3))->getY()<< ' '
                    << BoundName((triList[j]->pPtr((i+2)%3))->getBoundaryFlag())<< std::endl;
               goto error;
            }
         }
	 // check flip test
	 if( triList[j]->tPtr(i) != 0 )
	   {
	     switch(CheckForFlip( triList[j], i, false, false )) {
  ...
  error: ReportFatalError( "Error in mesh consistency." );
...
void tMesh<tSubNode>::setVoronoiVertices()
{
   ...
   {
      vector< double > xy( triList[j]->FindCircumcenter() );
      ...
      triList[j]->ePtr(0)->setRVtx( xy );
      triList[j]->ePtr(1)->setRVtx( xy );
      triList[j]->ePtr(2)->setRVtx( xy );

      if (0) {//DEBUG
	std::cout << "FOR edges: ";
	for( int i=0; i<=2; i++ ){
          std::cout << triList[i]->ePtr(i)->getID() << " ("
               << triList[i]->ePtr(i)->getOriginPtr()->getID() << ","
               << triList[i]->ePtr(i)->getDestinationPtr()->getID() << ") ";
	}
	std::cout << ", v verts are:\n";
	vector< double > xy_ = triList[j]->ePtr(0)->getRVtx();
...
// Quintijn & Arnaud's debug code
   ...
   if (0) { //DEBUG
     std::cout << "leaving hole defined by \n"
	  << "   Node  x  y " << std::endl;
	  for (int i= 0; i< static_cast<int>(nbrList.size()); i++){
	 std::cout << "   " << nbrList[i]->getID() << "     "
	      << nbrList[i]->getX() << "  "
	      << nbrList[i]->getY() << std::endl;
       }
   }
...
template< class tSubNode >
int tMesh< tSubNode >::
ExtricateNode( tSubNode *node, deque< tSubNode * > nbrList )
{
   if (0) //DEBUG
...
LocateTriangle( double x, double y, bool useFuturePosn)
{
   ...
   for (n=0 ;lv!=3 && lt; n++)
   {
      vector< double > xy1 = useFuturePosn ?
	lt->pPtr(lv)->FuturePosn():
	lt->pPtr(lv)->get2DCoords();
      vector< double > xy2 = useFuturePosn ?
	lt->pPtr( (lv+1)%3 )->FuturePosn():
	lt->pPtr( (lv+1)%3 )->get2DCoords();
      vector <double> XY; XY.push_back(x); XY.push_back(y);
      const double c =
	predicate.orient2d(xy1, xy2, XY);
...
ExtricateTriangle( tTriangle *triPtr )
{
   ...
   if( find(triList.begin(),triList.end(), triPtr ) == triList.end() )
     return 0;
...
template< class tSubNode >
int tMesh< tSubNode >::
RepairMesh( deque< tSubNode * > nbrList )
{
   if (0) //DEBUG
     std::cout << "RepairMesh: " << std::endl;
   if( nbrList.size() < 3 ) return 0;
   //nbrList.makeCircular();

   // Keep stitching until only 3 nodes are left
   typename deque<tSubNode *>::iterator it;
   it = nbrList.begin();
   while( nbrList.size() > 3 )
   {
      //std::cout << "in loop, nbr size = " << nbrList.getSize() << std::endl;
      //Xflowflag = 1;  // PURPOSE??
      if( Next3Delaunay( nbrList) ) //checks for ccw and Del.
      {
           //std::cout << "found 3 Delaun!\n";
         int ret = AddEdgeAndMakeTriangle( nbrList);
	 assert( ret );
	 ...
	 /* tSubNode * meshnodePtr = */
      ...
      it++;                    //step forward once in nbrList
   }
   assert( nbrList.size() == 3 );
   assert( ntri == static_cast<int>(triList.size()) );
   assert( nedges == edgeList.getSize() );
   assert( nnodes == nodeList.getSize() );       //make sure numbers are right
   int ret =    MakeTriangle( nbrList);             //make final triangle
   assert( ret );
...
AddEdge( tSubNode *node1, tSubNode *node2, tSubNode const *node3 )
{
   ...
   // error check
   vector< double > p0( node1->get2DCoords() ), p1( node2->get2DCoords() ),
...
template< class tSubNode >
int tMesh< tSubNode >::
AddEdgeAndMakeTriangle( deque< tSubNode *> nbrList)
{
   if (0) //DEBUG
...
template< class tSubNode >
int tMesh< tSubNode >::
MakeTriangle( deque< tSubNode *> nbrList)
{
   assert( nbrList.size() == 3 );
   tSubNode *cn, *cnn, *cnnn;
   cn = nbrList.front();      // cn, cnn, and cnnn are the 3 nodes in the tri
   cnn = nbrList[1];
   cnnn = nbrList[2];
   //nbrIter.Next(); ???
...
MakeTriangle( tSubNode *cn, tSubNode *cnn, tSubNode *cnnn )
{
   ...
   vector < double > p0( cn->get2DCoords() ), p1( cnn->get2DCoords() ),
       p2( cnnn->get2DCoords() );
   ...
   // initialized to zero.
   triList.push_back( new tTriangle( miNextTriID++, cn, cnn, cnnn ) );//put
   tTriangle *ct;
   ct = triList.back();            //ct now points to our new triangle
...
AddNode( tSubNode &nodeRef, kUpdateMesh_t updatemesh, double time,
	 kFlip_t flip )
{
   vector< double > xyz( nodeRef.get3DCoords() );
...
AttachNode( tSubNode* cn, tTriangle* tri )
{
  assert( tri != 0 && cn != 0 );
  int i;
  vector< double > xyz( cn->get3DCoords() );
  ...
  vector< double > p1( node1->get2DCoords() ),
    p2( node2->get2DCoords() ), p3( node3->get2DCoords() ),
    p4( node4->get2DCoords() );
  int colinearedg = -1;


  if( xyz.size() == 3) // why would this ever not be the case?
  ...
  if( colinearedg < 0 ) {
     ...
     const vector< double > p1( node1->get2DCoords() ), p2( node2->get2DCoords() ),
...
template< class tSubNode >
tSubNode *tMesh< tSubNode >::
AddNodeAt( vector< double > &xyz, double time )
{
   if (0) //DEBUG
   ...
   tTriangle *tri;
   if( xyz.size() == 3 ) tri = LocateTriangle( xyz[0], xyz[1] );
   ...
   if( xyz.size() != 3 ) tempNode.setNew2DCoords( xyz[0], xyz[1] );
   tempNode.setBoundaryFlag( kNonBoundary );
...
CheckLocallyDelaunay( double time )
{
  ...
  //put each triangle into the stack
  {
	...
	for( int i = 0; i < 3; i++ )
	  {
	    nodPtr = static_cast< tSubNode * >(triList[j]->pPtr(i));
	    if( nodPtr->isMobile() ) change = true;
	  }
	if( change ) triPtrList.insertAtBack( triList[j] );
      }
  }
...
CheckTriEdgeIntersect()
{
   ...
   while( flipped )
   {
      ...
      for (int l = 0; l < static_cast<int>(triList.size()); l++)
      {
         for( i=0; i<3; i++ )
         {
            cn = static_cast<tSubNode *>(triList[l]->pPtr(i));
            if( cn->isMobile() ) break;
         }
         if( i!=3 ) triptrList.insertAtBack( triList[l] );
      }
         ...
         if( !NewTriCCW( ct ) )
         {
            ...
            else
            {
		     ...
		     tSpkIter spokIter( cn );
                     vector<double> xy = cn->FuturePosn();
			   ...
			   if( ctop == 0 )
				...
				std::cout << "call FlipEdge from CTEI for edge between nodes "
                     ...
                     if( !crossed || ( crossed && ( !useFuturePosn || !no_edge ) ) )
                     {
                        ...
                        //delete the node;
			if (0) {//DEBUG
			  vector<double> xyz(cn->getNew3DCoords());
...
MoveNodes( double time, bool interpFlag )
{
   ...
   if( interpFlag &&
     ...
     nodeListIter_t nodIter( nodeList );
     for(cn=nodIter.FirstP(); nodIter.IsActive(); cn=nodIter.NextP()){
       const vector<double> newxy = cn->getNew2DCoords();
...
{
   deque< Point3D *> vvtxlist;  // List of V. vertex (x,y,z) coords at a node
   ...
   int size = vvtxlist.size();
   for (int i = 0; i< size ; i++)
   {
      //std::cout << "COORDS: x " << vvtxlist[i]->x << " y " << vvtxlist[i]->y << " z " << vvtxlist[i]->z << std::endl;
      tmpnode.set3DCoords( vvtxlist[i]->x, vvtxlist[i]->y, vvtxlist[i]->z );  // Assign to tmpnode
      //cn->TellAll();
...
void tMesh<tSubNode>::RenumberIDCanonically()
{
  ...
  deque < tTriangle * >  titer( getTriList() );     // tri list iterator
  const size_t nnodes = getNodeList()->getSize();  // # of nodes on list
  const size_t nedges = getEdgeList()->getSize();  // "    edges "
  const size_t ntri = getTriList().size();     // "    triangles "

  	
  {
    // First we set the Nodes Id in the order defined below
    // b1 <= b2 then x1 <= x2 then y1<=y2
    vector< tNode* > RNode; RNode.assign(nnodes,0);
    size_t i;
    tNode *cn;
    for( cn=niter.FirstP(), i=0; i<nnodes; cn=niter.NextP(), ++i )
      RNode[i] = cn;
	
	stable_sort(RNode.begin(), RNode.end(),orderRNode);
    const size_t s_ = RNode.size();
    for(i=0; i<s_; ++i){
      RNode[i]->setID(i);
	 }
    SetmiNextNodeID( RNode.size() );
  }
  ...
  {
    vector< tEdge* > REdge2; REdge2.assign(nedges/2,00);
    tEdge *ce;
    size_t i;
    ...
    stable_sort(REdge2.begin(), REdge2.end(),orderREdge);
    const size_t s_ = REdge2.size();
    for(i=0; i<s_; ++i) {
    ...
    SetmiNextEdgID( 2*REdge2.size() );
  }
  {
    // Set Triangle Id so that the vertexes are ordered
    vector< tTriangle* > RTri; RTri.assign(ntri,0);
    size_t i;
	int size = titer.size();
	for (int i=0; i< size; i++){
      titer[i]->SetIndexIDOrdered(); // set titer[i]->index_ in ID order
      RTri[i] = titer[i];
    }
    stable_sort(RTri.begin(), RTri.end(), orderRTriangle);
    const size_t s_ = RTri.size();
    for(i=0; i<s_; ++i)
      RTri[i]->setID(i);
    SetmiNextTriID( RTri.size() );
  }
}
...
template< class tSubNode >
int tMesh<tSubNode>::orderREdge(const tEdge *a_,const tEdge *b_ )
{
...
template< class tSubNode >
int tMesh<tSubNode>::orderRTriangle( tTriangle * a_, tTriangle * b_ )
{
  tTriangle *T1 = a_;
  tTriangle *T2 = b_;
...
InterveningTriangles( tNode* un, tNode* dn ) const
{
   ...
   vector< double > uvec(UnitVector( xsep, ysep ));
   tSpkIter sI( un );
   tEdge* ce;
   for( ce = sI.FirstP(); !sI.AtEnd(); ce = sI.NextP() )
