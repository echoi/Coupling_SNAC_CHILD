...
**
\*************************************************************************/
vector< double > FindIntersectionCoords( vector< double > xy1,
					  vector< double > xy2,
					  vector< double > xy3,
					  vector< double > xy4 )
{
...
double tNode::ComputeVoronoiArea()
{
  ...
  do
    {
      ...
      if (0) {//DEBUG
	vector< double > xy1 = ce->getRVtx();
	vector< double > xy2 = vedgList.getLast()->getPtr()->getRVtx();
	std::cout
	  << xy1.at(0) << " " << xy1.at(1) << "; "
  ...
  if( getBoundaryFlag() == kNonBoundary )
	...
	do
	  {
	    ...
	    for( ce=vtxIter.FirstP(); !( vtxIter.AtEnd() ); ce=vtxIter.NextP() )
	      {
		vector< double > xy = ce->getRVtx();
		vector< double > xyn = vtxIter.NextP()->getRVtx();
		vector< double > xynn = vtxIter.NextP()->getRVtx();
		...
		if( dy0 * dx1 > dx0 * dy1 ) //clockwise
		  {
		    vector< double > xynnn = vtxIter.NextP()->getRVtx();
			...
			vector<double>tmp; tmp.push_back(vx); tmp.push_back(vy);
			edgptr->setRVtx( tmp );
      ...
      {
	vector< double > xy2, xy3;
	deque< vector< double > > vcL; // list of vertex coordinates
	tEdge *ne, *nne;
	for( ce = vtxIter.FirstP(); !(vtxIter.AtEnd()); ce = vtxIter.NextP() )
	  {
	    ne = ce->getCCWEdg();
	    vector<double> xy1 = ne->getRVtx();
		...
		if( !PointsCCW( xy1, xy2, xy3 ) )
		  {
			...
			vector <double> tmp = FindIntersectionCoords( ce->getRVtx(), xy1, xy2, xy3 );
		    vcL.push_back(tmp);
		    nne = ne->getCCWEdg();
			vector < double> tmp2 = FindIntersectionCoords( xy1, nne->getRVtx(), xy2, xy3 );
		    vcL.push_back(tmp2);
		  }
		else vcL.push_back( xy1 );
	      }
	    else vcL.push_back( xy1 );
	  }
	...
	vector< double > xy = (vcL.front()); //ce = vtxIter.FirstP();
	//xy = ce->getRVtx();
	// Find out # of vertices in polygon:
	const int nverts = vcL.size(); //vedgList.getSize();
	for( i=1; i<=nverts-2; i++ )
	  {
	    double a, b, c;
	    vector<double> xyn = vcL[i]; //xyn = vtxIter.NextP()->getRVtx();// Vertex i
	    vector<double> xynn = vcL[++i];//vtxIter.ReportNextP()->getRVtx(); // Vertex i+1
	    {
...
**
\*******************************************************************/
void tNode::getVoronoiVertexList( deque <Point2D *> vertexList )
{
   assert( !boundary );
   ...
   do
   {
      vector<double> vtxarr = ce->getRVtx();
      Point2D * vtx = new Point2D( vtxarr.at(0), vtxarr.at(1) );
...
**
\*******************************************************************/
void tNode::getVoronoiVertexXYZList( deque <Point3D * > vertexList )
{
   tNode *n1, *n2;
   ...
   do
   {
      ...
      vector<double> vtxarr = ce->getRVtx();
      vector<double> zvals;
	  zvals.push_back(this->z);
	  zvals.push_back(n1->getZ());
	  zvals.push_back(n2->getZ());
      const double zz =
	PlaneFit(vtxarr.at(0), vtxarr.at(1), this->get2DCoords(),
		 n1->get2DCoords(), n2->get2DCoords(), zvals );
       Point3D * vtx = new Point3D(vtxarr.at(0), vtxarr.at(1), zz);
      if (0) //DEBUG
...
tTriangle::FindCircumcenter() const
{
   ...
   vector< double > xyo; pPtr(0)->get2DCoords( xyo );
   vector< double > xyd1; pPtr(1)->get2DCoords( xyd1 );
   vector< double > xyd2; pPtr(2)->get2DCoords( xyd2 );
...
{
  tTriangle* ct = 0;
  vector< double > xy0; xy0.push_back(x); xy0.push_back(y);
  vector< double > xy1;
  vector< double > xy2;
  vector< double > xy3;
  p[0]->get2DCoords( xy1 );
  ...
  double c = predicate.orient2d( xy1,xy2,xy0 );
  if( c < 0.0 )
    return ct = t[2];
  c = predicate.orient2d( xy2, xy3, xy0 );
  if( c < 0.0 )
    return ct = t[0];
  c = predicate.orient2d( xy3, xy1, xy0 );
  if( c < 0.0 )
    return ct = t[1];
  return ct;
}
...
{
  vector<double> P; P.push_back(x); P.push_back(y);
  vector<double> P0; P0.push_back(p[0]->getX()); P0.push_back( p[0]->getY());
  vector<double> P1; P1.push_back(p[1]->getX()); P1.push_back( p[1]->getY());
  if (predicate.orient2d(P0, P1, P) <0)
    return false;
  vector <double> P2; P2.push_back(p[2]->getX()); P2.push_back(p[2]->getY());
