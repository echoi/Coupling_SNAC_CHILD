...
public:
  ...
  vector< double > get3DCoords() const;      // returns x,y,z
  vector< double > get2DCoords() const;      // returns x,y
// SL, 7/2003: added less costly versions that pass references
  void get3DCoords( vector< double >  ) const;
  void get2DCoords( vector< double >& ) const;
  ...
  void getVoronoiVertexList( deque <Point2D *> );  // Returns list of V vertices
  void getVoronoiVertexXYZList( deque <Point3D *> ); // As above plus interp z
...
public:
  ...
  inline vector< double > getRVtx() const;  // returns Voronoi vertex for RH triangle
  inline void getRVtx( vector< double > ) const; // less costly ref-passing version
  ...
  void setRVtx( vector< double >);  // sets coords of Voronoi vertex RH tri
...
private:
  ...
  vector< double > rvtx; // (x,y) coords of Voronoi vertex in RH triangle
...
public:
  ...
  vector<double> FindCircumcenter() const; // computes & returns tri's circumcenter
...
inline void tNode::get3DCoords( vector< double > xyz ) const
{
   if( xyz.size() != 3 ) xyz.assign(3,0);
   xyz[0] = x;
   xyz[1] = y;
   xyz[2] = z;
}
...
inline void tNode::get2DCoords( vector< double >& xy ) const
{
   if( xy.size() != 2 ) xy.assign(2,0);
   xy.at(0) = x;
   xy.at(1) = y;
}
...
/*******************************************************************\
  ...
  5/2003 AD
\*******************************************************************/
inline vector< double > tNode::FuturePosn() {return get2DCoords();}
...
inline void tEdge::getRVtx( vector< double > arr ) const
{
   arr = rvtx;
}
...
inline void tEdge::setRVtx( vector< double > arr )
{
   if (0)//DEBUG
