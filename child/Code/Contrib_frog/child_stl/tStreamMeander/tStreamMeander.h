...
public:
   ...
   vector< double > FindInterpCoords( tLNode*, tLNode* );
   tLNode* FindShortcutNode( tLNode*, tLNode* );
   tLNode* BlockShortcut( tLNode*, tLNode*, tLNode&, vector<double>&, double );
   ...
   void FindBankCoords( tLNode*, vector< double >& ); // carved from Make...
   void ResetEffNbrCoords( tLNode * );
   //finds the erodibility of each bank, returns an array [right, left]:
   vector< double > FindBankErody( tLNode * ) const;
...
#undef FIXCRITFLOWBUG
   ...
   double Pdz; //dependence of bank erody on bank height, 0=none, 1=all
    vector< int > nrnodes; //array w/ #elements=#reaches, # "active" reach nodes
    vector< double > reachlen; // " , length of active reach in reach list
    vector< double > taillen;  // " , length of inactive "tail" in reach list
    tRand *rand;
};
