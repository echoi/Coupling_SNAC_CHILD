...
void forcelag_(const integer *stnserod, const integer *stations, 
	       const doublereal *rho, vector <double> vel,
	       vector <double> depth, vector <double> width,
	       vector <double> latforce, vector <double> lag,
	       vector <double> curvature, vector <double> acs,
	       vector <double> dels, vector <double> deln);
static
void forcedist_(const integer *stnserod, const integer *stations, 
		vector <double> lambda, vector <double> width,
		vector <double> lag, vector <double> latforce,
		vector <double> dels, vector <double> phi,
		vector <double> curvature, vector <double> depth,
		vector <double> spreaddelta_x__, vector <double> spreaddelta_y__,
		vector <double> rightdepth, vector <double> leftdepth, 
		vector <double> xs, vector <double> tauwall);
static
void initialize_(const integer *stnserod, vector <double>  phi, 
		 vector <double> x, vector <double> y,
		 vector <double> delx, vector <double> dely, 
		 doublereal *rho, doublereal *grav,
		 vector <double> width, vector <double> lambda);
static
void changeposition_(const integer *stnserod, const integer * /*stations*/, 
		     vector <double> lerody, vector <double> rerody,
		     vector <double> spreaddelta_x__,
		     vector <double> spreaddelta_y__,
		     vector <double> delx, vector <double> dely,
		     vector <double> depth,
		     vector <double> delta_x__, vector <double> delta_y__);
static
void channel_(const integer *stnserod, const integer *stations, 
	      vector<double> flow, vector<double> slope,
	      vector<double> diam, vector<double> width,
	      const doublereal * /*rho*/, const doublereal * /*grav*/,
	      vector <double> phi, vector <double> curvature,
	      vector<double> dels, 
	      vector <double> acs, vector <double> deln, vector<double> rightdepth,
	      vector<double> leftdepth, vector<double> depth, 
	      vector <double> vel, vector <double> delx, vector <double> dely,
	      doublereal *transfactor, vector <double> transslope);
static
doublereal angle_(const doublereal *, const doublereal *);
static
void getcurv_(const integer *, const integer *, vector <double> , 
...
void meander_(const integer *stations, const integer *stnserod, 
	      vector<double> x, vector<double> y,
	      vector<double> xs, vector<double> dels, 
	      vector<double> flow, vector<double> rerody,
	      vector<double> lerody, vector<double> slope,
	      vector<double> width, vector<double> depth, vector<double> diam, 
	      vector<double> delta_x__, vector<double> delta_y__,
	      vector<double> rightdepth, vector<double> leftdepth,
	      vector<double> lambda)
{
    ...
    vector<double>
      ...
      phi(*stnserod,0),
      tauwall(*stnserod,0),
      spreaddelta_x__(*stnserod,0),
      spreaddelta_y__(*stnserod,0),
      curvature(*stnserod,0),
      vel(*stnserod-1,0),
      transslope(*stnserod-1,0),
      acs(*stnserod,0),
      deln(*stnserod,0),
      latforce(*stations,0),
      lag(*stations,0);
...
/*     declare parameter values; get initial channel config.: */
    ...
    initialize_(stnserod, phi, x, y, delx, dely, &rho, &grav, width, lambda);
/*     print *, 'stnserod in meander:', stnserod */

/*     define channel characteristics: */

    channel_(stnserod, stations, flow, slope, diam, width, &
	    rho, &grav, phi, curvature, dels, acs, deln, rightdepth, 
	    leftdepth, depth, vel, delx, dely, &transfactor, 
	    transslope);

/*     calculate lateral force and downstream lag: */

    forcelag_(stnserod, stations, &rho, vel, depth, width, latforce, 
	    lag, curvature, acs, dels, deln);

/*     propagate and smooth lateral force: */

    forcedist_(stnserod, stations, lambda, width, lag, latforce, 
	    dels, phi, curvature, depth, spreaddelta_x__, 
	    spreaddelta_y__, rightdepth, leftdepth, xs, tauwall);

/*     change channel position: */

    changeposition_(stnserod, stations, lerody, rerody, 
	    spreaddelta_x__, spreaddelta_y__, delx, dely, depth, 
	    delta_x__, delta_y__);
...
/*  Acs is cross-sectional area of the half-channel at the inside of the bend */
    ...
    getcurv_(stnserod, stations, delx, dely, dels, curvature);
...
void getcurv_(const integer *stnserod, const integer * /*stations*/, 
	      vector <double> delx, vector <double> dely,
	      vector <double> dels, vector <double> curvature)
{
    /* System generated locals */
    integer i__1;
    doublereal d__1;
