                                       ...
                                       vector<double>& ic, double time )
{
   ...
   vector< double > dumcoords(4,0);
   dumcoords[0] = ic[0];
   dumcoords[1] = ic[1];
   vector< double > checkZOld = bpn->getZOld();
...
vector< double > tStreamMeander::FindInterpCoords( tLNode* crn, tLNode* nPtr )
{
   ...
   else
   {
      ...
      vector< double > xp(npts,0), yp(npts,0), zp(npts,0);
      vector< double > ttic( 3*(npts-1),0 );
      const double x0 = crn->getX();
      const double y0 = crn->getY();
      const double z0 = crn->getZ();
      const double x1 = nPtr->getX();
   ...
   for( creach = rlIter.FirstP(), i=0; !rlIter.AtEnd(); creach = rlIter.NextP(), i++ )
   {
      ...
      for( crn = rnIter.FirstP(), j=0; j<nrnodes[i]; crn = rnIter.NextP(), ++j )
      {
         ...
         if( curseglen > maxseglen )
         {
            ...
            vector< double > ic = FindInterpCoords( crn, nPtr );
            ...
            vector< double > checkZOld = crn->getZOld();
            if( ( checkZOld[0] != 0.0 || checkZOld[1] != 0.0 )
                && bpn != NULL ){
...
void tStreamMeander::FindReaches()
{
   ...
   if( reachList.getSize() == 0 ) return;
      vector< int > iArrPtr( reachList.getSize(),0 );
   {
      vector< double > fArrPtr(reachList.getSize(),0 );
      reachlen = fArrPtr;
      taillen = fArrPtr;
   }
   if (0) //DEBUG
        ...
        creach = rlIter.NextP(), i++ )
   {
      ...
      rnIter.Reset( *creach );
      {
             ...
             //initialize deltax, deltay, newx, newy:
         {
            ...
            if (0){ //DEBUG
               vector< double > newxy = curnode->getNew2DCoords();
      ...
      vector< double >
          xa(nttlnodes,0), ya(nttlnodes,0), xsa(nttlnodes,0), qa(nttlnodes,0),
          rerodya(nttlnodes,0), lerodya(nttlnodes,0), delsa(nttlnodes,0),
          slopea(nttlnodes,0), widtha(nttlnodes,0), deptha(nttlnodes,0),
          diama(nttlnodes,0), deltaxa(nttlnodes,0), deltaya(nttlnodes,0),
          rdeptha(nttlnodes,0), ldeptha(nttlnodes,0), lambdaa(nttlnodes,0);

      {
              ...
              curnode = rnIter.NextP(), j++ )
         {
            ...
            vector< double > bankerody = FindBankErody( curnode );
...
#undef POINTBARFIX
        ...
        creach = rlIter.NextP(), i++ )
   {
      ...
      for( curnode = rnIter.FirstP(), j=0; j<num;
           curnode = rnIter.NextP(), j++ )
      {
         vector< double > delta = curnode->getLatDisplace();
...
**
\***************************************************************/
void tStreamMeander::FindBankCoords( tLNode* cn, vector< double >& posRef )
{
   assert( posRef.size() >= 4 );
   vector< double > cnpos = cn->get2DCoords();
   
   //find coordinates of banks, i.e., coords at n = + and -width/2
   tLNode* cnbr = cn->getDownstrmNbr();
   vector< double > dsnpos = cnbr->get2DCoords();
   double x0 = cn->getX();
   double y0 = cn->getY();
   double x1 = cnbr->getX();
   double y1 = cnbr->getY();
        ...
        cr = rlIter.NextP(), i++ )
   {
           ...
           cn = rnIter.NextP(), j++ )
      {
         ...
         if( cn->DistFromOldXY() >= width * ( 0.5 + leavefrac ) )
         {
            ...
            vector< double > oldpos = cn->getXYZD();
            //find coordinates of banks, i.e., coords at n = + and -width/2
...
void tStreamMeander::ResetEffNbrCoords( tLNode *nPtr )
{
   ...
   vector< double > xy1 = nPtr->get2DCoords();
   tLNode* nNPtr = nPtr->getDownstrmNbr();
   vector< double > xy2 = nNPtr->get2DCoords();
   vector< double > xy1n = nPtr->getNew2DCoords();
   ...
   tSpkIter sI(nPtr);
   vector< double > spD( sI.getNumSpokes() * 2,0 ),
       spR( sI.getNumSpokes() * 2,0 );
   ...
   do
   {
      vector< double > xy = ce->getDestinationPtrNC()->get2DCoords();
...
tStreamMeander::FindBankErody( tLNode *nPtr ) const
{
   ...
   tLNode *nNPtr = nPtr->getDownstrmNbr();
   vector< double > xyz1 = nPtr->get3DCoords();
   vector< double > xy2 = nNPtr->get2DCoords();
   vector< double > dxy(2,0);
   dxy[0] = xy2[0] - xyz1[0];
   dxy[1] = xy2[1] - xyz1[1];
   double a = dxy[0];
   double b = dxy[1];
   ...
   do
   {
      ...
      vector< double > xy = cn->get2DCoords();
      const double d = a * xy[0] + b * xy[1] + c;
      spD[i] = spD[n + i] = d;
      //if d=0 then point is on line
      spR[i] = spR[n + i] = DistanceToLine( xy[0], xy[1], a, b, c );
      ...
      if( n > 0 )
      {
         ...
         for( ce = sI.FirstP(); !(sI.AtEnd()); ce = sI.NextP() )
         {
            ...
            if( mn->Meanders() )
            {
               ...
               vector< double > xyn = mn->getNew2DCoords();
               xp = xyn[0];
               yp = xyn[1];
               ...
               {
                  vector< double > xy = mn->get2DCoords();
                  xp = xy[0];
                  yp = xy[1];
            ...
            if( ft )
            {
               ...
               if( pointtodelete != 0 )
               {
                  ...
                  else
                  {
                     ...
                     vector< double > xy0 = cn->getNew2DCoords();
                     if( meshPtr->LocateTriangle( xy0[0], xy0[1] ) == 0 )
                         cn->RevertToOldCoords();
                     vector< double > xy1 = dscn->getNew2DCoords();
