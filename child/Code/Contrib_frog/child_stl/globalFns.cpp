...
*/
/**************************************************************************/
vector< double > UnitVector( tEdge const * ePtr )
{
   assert( ePtr != 0 );
   vector< double > oxy( ePtr->getOriginPtr()->get2DCoords() );
   vector< double > dxy( ePtr->getDestinationPtr()->get2DCoords() );
   const double dx = dxy[0] - oxy[0];
   const double dy = dxy[1] - oxy[1];
   const double mag = sqrt( dx * dx + dy * dy );
   vector< double > tmp;
   tmp.push_back( dx/mag);
   tmp.push_back(dy/mag);
   return tmp;
     
}

vector< double > UnitVector( double dx, double dy )
{
   const double mag = sqrt( dx * dx + dy * dy );
   vector< double > tmp; 
   tmp.push_back( dx/mag);
...
*/
/**************************************************************************/
vector< double > NewUnitVector( tEdge * ePtr )
{
   assert( ePtr != 0 );
   vector< double > oxy( ePtr->getOriginPtrNC()->FuturePosn() );
   vector< double > dxy( ePtr->getDestinationPtrNC()->FuturePosn() );
   const double dx = dxy[0] - oxy[0];
   const double dy = dxy[1] - oxy[1];
   const double mag = sqrt( dx * dx + dy * dy );
   vector< double > tmp;
...
*/
/**************************************************************************/
double FindCosineAngle0_2_1( vector< double > const &p0,
...
*/
/***************************************************************************/
int TriPasses( vector< double > ptest,
               vector< double > p0,
               vector< double > p1,
               vector< double > p2 )
{
   if (0) //DEBUG
     std::cout << "TriPasses? ";
#if 1
   double ans = predicate.incircle( p0, p1,
				    p2, ptest);
   return ans > 0. ? 0 : 1;
#else
   double dx0, dx1, dy0, dy1;
...
*/
/***************************************************************************/
bool PointsCCW( vector< double > p0,
		vector< double > p1,
		vector< double > p2 )
{
   if (0) //DEBUG
   ...
   return ( predicate.orient2d( p0, p1, p2 ) > 0 );
}
...
*/
/***************************************************************************/
int Orientation( vector< double > p0,
                 vector< double > p1,
                 vector< double > p2 )
{
   //NEW: call exact arithmetic predicate:
   const double ans = predicate.orient2d( p0, p1, p2 );
   if( ans > 0. ) return 1;
   if( ans < 0. ) return -1;
   return 0;
}
...
*/
/***************************************************************************/
int InNewTri( vector< double > xy, tTriangle const *ct )
{
   int j;
   tLNode *vtx;
