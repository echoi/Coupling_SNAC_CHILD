...
public:
  ...
  const vector< double > getGrade( ) const;
  int getNumg() const;
  ...
  void EroDepSimple(int, vector<double>, double, double);         // layer, dh[i], time, paleocurrent

  vector<double> addtoLayer(int, double,double);  // layer, g, fill[g], tt
  void addtoLayer(int, int, double, double, double);

  void makeNewLayerBelow(int, tLayer::tSed_t, double, vector<double>,
			 double, double);
...
private:
  ...
  int nWrite;
  vector<int> test;
  vector<int> section;		          // Vector with 10 section locations, 5x, and 5y
  vector<double>surface;		 // timeslice specific surface area
  vector<double>subsurface;              // timeslice specific subsurface cummulative height in the entire floodplain
  vector<double>subsurface_mbelt;        // timeslice specific subsurface cummulative height in meander belt
  vector<double>outputTime;              // vector with all the exact timings of the output /writesteps
};
