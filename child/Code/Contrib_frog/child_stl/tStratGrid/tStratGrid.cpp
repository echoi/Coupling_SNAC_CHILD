    ...
    imax(0), jmax(0)    
{
  ...
  surface.assign(arraysize+1,0);           // array with timeslice specific surface area - 
  subsurface.assign(arraysize+1,0);        // array with timeslice specific subsurface cummulative height - 
  subsurface_mbelt.assign(arraysize+1,0);  // same, but for zone close to the channel
  outputTime.assign(arraysize+1,0);        // array with the storm dependent output times - 
  ...
  for(k=0;k<5;k++){
    section.push_back(int(0.5*stepx + k*stepx));
    std::cout <<"X-Section "<<k+1<<" at i= " <<section[k] <<" or X= "<<(*StratNodeMatrix)(section[k],0).getX()<< '\n';
  }
  for(k=5;k<10;k++){
    section.push_back(int(0.5*stepy + (k-5)*stepy));
...
void tStratGrid::updateConnect()
{
  ...
  //
  deque < tTriangle * > triList( mp->getTriList() );
  for (int i = 0; i < static_cast<int>(triList.size()) ; i++){
    // Find box containing the current triangle
    const TriBox thisBox( triList[i], xcorner, ycorner, griddx );
	...
	for(int j=bjmin; j<=bjmax; ++j) {
	  if ((*StratConnect)(i,j) != NULL)
	    continue;
	  if (triList[i]->containsPoint( (*StratNodeMatrix)(i,j).getX(),
				 (*StratNodeMatrix)(i,j).getY() )
	      ){
	    (*StratConnect)(i,j) = triList[i];
	  }
	}
      }
    }
...
void tStratGrid::InterpolateElevations()
{
  ...
  for(i=0; i<imax; i++){
	...
	// and interpolate a new z value
    	...
    	vector<double> tri_elevs;
		tri_elevs.push_back(lnds[0]->getZ());
		tri_elevs.push_back(lnds[1]->getZ());
		tri_elevs.push_back(lnds[2]->getZ());
...
void tStratGrid::InterpolateErodep(double time)
{
  ...
  for(i=0; i<imax; i++){
	...
	vector<double>Dh; Dh.push_back(2); Dh.push_back(2);
	vector<double> tri_CoarseDh;
	tri_CoarseDh.push_back(lnds[0]->getAccCoarse());
	tri_CoarseDh.push_back(lnds[1]->getAccCoarse());
	tri_CoarseDh.push_back(lnds[2]->getAccCoarse());
	...
	vector<double> tri_FineDh;
	tri_CoarseDh.push_back(lnds[0]->getAccFine());
	tri_CoarseDh.push_back(lnds[1]->getAccFine());
	tri_CoarseDh.push_back(lnds[2]->getAccFine());
...
double tStratGrid::CalculateMeanderCurrent( tTriangle *tri, double sx, double sy) const
{
  ...
  case 3:     // There are three surrounding meander nodes
    {
      vector<double> CompassAngles;
	  CompassAngles.push_back(CompassAngle(lnds[0],lnds[0]->getDownstrmNbr()));
	  CompassAngles.push_back(CompassAngle(lnds[2],lnds[2]->getDownstrmNbr()));
...
//vector<double> tStratNode::grade = 1;
...
void tStratNode::setGrade( int i, double size ) 
{
  if(i>=numg)
...
const vector< double > tStratNode::getGrade( ) const
{
  return grade;
}
...
void tStratNode::EroDepSimple( int l, vector<double>dh, double tt,double current)
{
  int debugI,debugJ;
  int g;
 ...
 \************************************************************/
  if( (dh[0] > 0.0 || dh[1] > 0.0) && dhtotal > 0.0){
    ...
    if(getLayerSed(l) != 0 && getLayerDepth(l) > 0.0){
	  ...
	  ratio.push_back(dh[0]/dhtotal);                         // fraction coarse in added dh
	  ratio.push_back(dh[1]/dhtotal);                         // fraction fine
 ...
 ******************************************************************/
vector<double> tStratNode::addtoLayer(int l, double val, double tt)
{
  ...
  if(hlp->getDepth()+val>1e-7)
    {
      ...
      while(n<numg)
	{
	  ret.push_back(hlp->getDgrade(n)*val/amt);             // return this, will be passed on to top layer
  ...
  else
    {
      ...
      while(n<numg)
	{
	  ret.push_back(-1*hlp->getDgrade(n));
	  n++;
	}
      removeLayer(l);
      return ret;
...
/*****************************************************************
 ** tStratNode::makeNewLayerBelow(int i, int sd, double erd, vector<double> sz, double tt)
