...
public1(-1)
{
  ...
  vector<double> dgradehelp;
  vector<double> dgradebrhelp;
  ...
  if(!optReadLayer){
    ...
    dgradehelp.assign( numg,0 );
    dgradebrhelp.assign( numg,0 );
    sum = 0;
    sumbr = 0;
    size_t i=0;
    add[0]='1';
    ...
    if (0) //DEBUG
      std::cout << layerlist.size() << " layers created " << std::endl;
  }
...
void tLNode::TellAll() const
{
  ...
  std::cout << "first: " << &(layerlist.front()) << std::endl;
  std::cout << "last: " << &(layerlist.back())  << std::endl;
}
#endif
...
void tLNode::addQsin( vector< double > const &val )
{
  int i;
...
void tLNode::addQsdin( vector< double > const &val )
{
  int i;
  const int n = val.size();
  for(i=0; i<n; i++){
...
void tLNode::addQs( vector< double > const & val )
{
...
void tLNode::PrepForMovement( tTriangle const* tri, double time ){
  vector<double> newxy = getNew2DCoords();
...
void tLNode::LayerInterpolation( tTriangle const* tri, double tx, double ty, double time )
{
  ...
  deque< tLayer *> helplist; //Make the layer list first.  When
  //the list is made, then set the nodes layerlist equal to helplist.

  if( numnodes==3 ){
    vector<int> layindex; layindex.assign(3,0);//What layer are you at for each node
    vector<double> dep; dep.assign(3,0);//The depth of the layer with the matching age
    vector<double> age; age.assign(3,0);//The age of the current layer at each node
    //The sediment flag of the current layer at each node
    vector<tLayer::tSed_t> sed; sed.assign(3,0); sed.push_back(tLayer::kBedRock);
    ...
    tLayer * layhelp = new tLayer(); //set values in this layer then insert in back of layerlist
    layhelp->setDgradesize(numg);
    ...
    //around the node
    vector<double> elevs;
	elevs.push_back(lnds[0]->getZ());
	elevs.push_back(lnds[1]->getZ());
	elevs.push_back(lnds[2]->getZ());
    ...
    /*std::cout<<"new z "<<getZ()<<" theoretical z "<<theoryz<<" diff "<<diff<<std::endl;
      ...
      std::cout<<"numlayers = "<<helplist.size()<<std::endl;
      std::cout<<"depth of first layer = "<<helplist.front()->getDepth()<<std::endl;*/

    while(diff>0){
      if(helplist.front()->getDepth()<diff){
	diff-=helplist.front()->getDepth();
	helplist.pop_front();
      }
      else{
	double dpth = helplist.front()->getDepth();
	tLayer * firstlay = helplist.front();
    ...
    vector<int> layindex; layindex.assign(2,0);//What layer are you at for each node
    vector<double> age; age.assign(2,0);//The age of the current layer at each node
    //The sediment flag of the current layer at each node
    vector<tLayer::tSed_t> sed; sed.assign(2,0); sed.push_back(tLayer::kBedRock);
  ...
  if(maxregdep-helplist.front()->getDepth()>0.001 &&
     helplist[1]->getSed()>0){
    ...
    helplist.push_front(firstlay);
  }
  else if(helplist.front()->getRtime()<0){
    ...
    layhelp->setCtime(time);
    layhelp->setRtime(0.);
    layhelp->setDgrade(0,maxregdep*helplist.front()->getDgrade(0)/helplist.front()->getDepth());
    if(numg>1)
      layhelp->setDgrade(1,maxregdep*helplist.front()->getDgrade(1)/helplist.front()->getDepth());
    layhelp->setErody(helplist.front()->getErody());
    helplist.front()->setDepth( helplist.front()->getDepth()-maxregdep);
    helplist.push_front( layhelp );
...
/*******************************************************************\
  ...
  5/2003 AD
\*******************************************************************/
vector< double > tLNode::FuturePosn() {
...
tNode *tLNode::splitFlowEdge() {
  ...
  const vector< double > zeroArr(4,0);
...
vector<double> tLNode::EroDep( int i, vector<double> valgrd, double tt)
{
  double amt, val, olddep;
  vector<double> update(numg,0);
  vector<double> hupdate(numg,0);
 ...
 ******************************************************************/
vector<double> tLNode::addtoLayer(int i, double val)
{
  assert( val<0.0 ); // Function should only be called for erosion

  vector<double> ret(numg,0);
...
void tLNode::makeNewLayerBelow(int i, tLayer::tSed_t sd, double erd,
			       vector<double> const &sz, double tt)
{
  tLayer * hlp = new tLayer();
  size_t n;
  hlp->setCtime(tt);
  hlp->setRtime(tt);
  hlp->setEtime(0.);
  ...
  if(i>=0){
	...
	layerlist.insert(layerlist.begin()+i,hlp);
    //layerlist.insertAtNext(hlp, layerlist.getListNode(pt));
  }
  else{
    layerlist.push_front(hlp);
  }
}
...
{
  const double sz = layerlist.size();
  int i=0;
  double elev = 0;
  while(i<sz)
    {
...
{
  vector< double > const &oldpos =  chan.migration.xyzd;
