...
public:
  ...
  inline vector< double > const & getDgrade() const;
...
protected:
  ...
  vector< double > dgrade; // depth of each size if multi size
  double paleocurrent;
};
...
private:
  ...
  vector< double > dgrade;/* depth of each sediment class in active layer [m] */
};
...
public:
  ...
  inline vector< double > getZOld() const;
  inline vector< double > getNew2DCoords() const;   //for chan.migration.newx, newy
  inline void setNew2DCoords( double, double );      //        "
  inline vector< double > getNew3DCoords() const;   //        "
  inline vector< double > getLatDisplace() const;  //for chan.migration.deltax, deltay
  ...
  inline vector< double > const & getAlluvThicknessm( ) const;
  ...
  vector<double> EroDep(int, vector<double>, double);
  ...
  inline void IncrementAccummulatedDh(vector<double>&);  // increment dh's, called during tFloodplain functions
  inline const vector<double>& getAccummulatedDh() const;      // returns array with current values
  ...
  vector<double> addtoLayer(int, double);
  ...
  void makeNewLayerBelow(int, tLayer::tSed_t, double, vector<double> const &,
			 double);
  ...
  virtual vector< double > FuturePosn();
...
protected:
  ...
  vector< double > qsm;             /* multi size; transport rate of each size fraction*/
  double qsin;                      /* Sediment influx rate*/
  vector<double> qsinm;           /* multi size; influx rate of each size fraction*/
  double qsdin;                      /* Sediment influx rate from diffusion*/
  vector< double > qsdinm;           /* multi size; influx rate of each size fraction*/
  double uplift;                    /* uplift rate*/
  deque< tLayer *> layerlist;        /* list of the different layers */
  tStratNode *stratNode;            /* Pointer to rectangular grid node. */
  vector< double > accumdh;         /* temp inremental record of dh-fine and dh-coarse, used for projection on the tStratGrid */
  static size_t numg;
...
{
    vector < double > tmp;
	tmp.push_back( chan.migration.zoldright);
	tmp.push_back(chan.migration.zoldleft);
	return tmp;
}
...
tLNode::getNew2DCoords() const
{
  if (Meanders()){
    vector< double >tmp;
	tmp.push_back(chan.migration.newx);
	tmp.push_back(chan.migration.newy);
	return tmp;
	}
	vector< double >tmp;
	tmp.push_back(x);
...
inline vector< double >                                                   //tNode
tLNode::getNew3DCoords() const
{
	vector <double> tmp1; tmp1.push_back(chan.migration.newx); tmp1.push_back(chan.migration.newy); tmp1.push_back(z);
	vector <double> tmp2; tmp2.push_back(x); tmp2.push_back(y); tmp2.push_back(z); 
  return Meanders() ?
    tmp1 : tmp2;
}
...
inline vector< double > tLNode::
getLatDisplace() const
{
    vector< double > tmp;
	tmp.push_back( chan.migration.deltax);
	tmp.push_back(chan.migration.deltay);
	return tmp;
}
...
{
  if(unlikely( (i>=numg) || (i>=qsinm.size()) )) {
...
inline void tLNode::setQsin( vector< double > const & q_ )
{
  assert( qsinm.size() == q_.size() );
...
{
  if(unlikely( (i>=numg) || (i>=qsdinm.size()) )) {
...
inline void tLNode::setQsdin( vector< double > const & q_ )
{
  assert( qsdinm.size() == q_.size() );
...
inline void tLNode::setGrade( size_t i, double size )
{
  if(unlikely(i>=numg))
...
inline void tLNode::setXYZD( vector< double > const &arr )
{
  chan.migration.xyzd = ( arr.size() == 4 ) ? arr : vector< double >(4,0);
}
...
inline void tLNode::IncrementAccummulatedDh(vector<double> &dh)
{
  accumdh[0]+= dh[0];
  accumdh[1]+= dh[1];
}

inline const vector<double>& tLNode::getAccummulatedDh() const
{
  return accumdh;
}
