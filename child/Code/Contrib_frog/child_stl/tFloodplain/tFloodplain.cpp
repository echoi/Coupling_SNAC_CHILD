  ...
  meshPtr(mp)
{
   ...
   deparr.assign(numg,0); // dimension & init to 0 the deparr array
   deparrRect.assign(numg+1,0);
...
void tMainChannelDriver::RaiseBanks(double future_bed, tLNode *cn, tLNode *upstrN, double tm )
{
  ...
  vector<double> delz; delz.assign( num_grnsize_fractions,0 );
  vector<double> delzRect; delzRect.assign( num_grnsize_fractions + 1,0);
						     ...
						     tLNode * inletNode )
{
  ...
  vector<double> delz; delz.assign( num_grnsize_fractions,0 );           // for the triangular nodes
  vector<double> delzRect; delzRect.assign( num_grnsize_fractions + 1,0);    // CAREFUL, temp Hack by Q., this way you have 2 classes in StratGrid, but only one in the mesh
