...
private:
  ...
  vector< double > x;      // node x coords
  vector< double > y;      // node y coords
  vector< double > z;      // node z values
  vector< int > edgid;     // node edge ID #s
  vector< int > boundflag; // node boundary codes
  vector< int > orgid;     // directed edge origin node ID #s
  vector< int > destid;    // directed edge destination node ID #s
  vector< int > nextid;    // ID #s of next counter-clockwise edges
  vector< int > p0;     // IDs of triangle node 0
  vector< int > p1;     // IDs of triangle node 1
  vector< int > p2;     // IDs of triangle node 2
  vector< int > e0;     // IDs triangle clockwise-oriented edge 0
  vector< int > e1;     // IDs triangle clockwise-oriented edge 1
  vector< int > e2;     // IDs triangle clockwise-oriented edge 2
  vector< int > t0;     // IDs of neighboring tri's opposite node 0
  vector< int > t1;     // IDs of neighboring tri's opposite node 1
  vector< int > t2;     // IDs of neighboring tri's opposite node 2
...
tListInputDataMesh( const tInputFile &infile )
{
  ...
  y.assign( nnodes,0 );
  z.assign( nnodes,0 );
  edgid.assign( nnodes,0 );
  boundflag.assign( nnodes,0 );
  orgid.assign( nedges,0 );
  destid.assign( nedges,0 );
  nextid.assign( nedges,0 );
  p0.assign( ntri,0 );
  p1.assign( ntri,0 );
  p2.assign( ntri,0 );
  e0.assign( ntri,0 );
