...
vector< double > tStreamNet::getInSedLoadm( ) const {return inlet.inSedLoadm;}
...
vector< double > tStreamNet::getInletSedSizeFraction() const {return inlet.inletSedSizeFraction; }
   ...
   if( inletbc )
   {
     ...
     lastdigit = '0';
      ...
      if( optCalcSedFeed ) 
      {
        ...
        inletSedSizeFraction.assign(numg,0);
      }
      else
      {
        inletSlope = 0;
        inletSedSizeFraction.assign(0,0);
      }
...
void tInlet::FindNewInlet()
{
   ...
   const vector< double > xyz( innode->get3DCoords() );
   yin = xyz[1];
   zmin = xyz[2];
   newinnode = innode;
     //for( cn = nI.FirstP(); nI.IsActive(); cn = nI.NextP() )
  ...
  char astring[12];   // string var used in reading grain-size classes
   ...
   if( miNumGrainSizeClasses>1 ) // In this case we will calc D50 on the fly
	 ...
	 * pow( kt / (  thetac*(sigma-rho)*grav*shearRatio ), 1.0 / alpha );
       mdPPexp2 = -1.0 / alpha;
       mD50BySizeClass.assign( miNumGrainSizeClasses,0 );
