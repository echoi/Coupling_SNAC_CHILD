...
public:
    ...
    vector <double > getInSedLoadm() const;
...
private:
    ...
    vector< double > inSedLoadm; // incoming sediment load if multi-sizes
    ...
    vector<double> inletSedSizeFraction;  // Proportion of each sediment class on bed at inlet
...
class tParkerChannels
{
 ...
 private:
  ...
  vector<double> mD50BySizeClass; // Array recording D50 of each size class
...
public:
    ...
    vector< double > getInletSedSizeFraction() const;  // Returns sed size fractions on bed of inlet node if appl.
