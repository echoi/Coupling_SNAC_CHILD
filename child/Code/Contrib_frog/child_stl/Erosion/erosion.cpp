...
double tEquilibCheck::FindIterChngRate()
{
  ...
  vector< double > * tmp; 
  tmp->push_back( timePtr->getCurrentTime()); 
  tmp->push_back(ma);

  if( massList.size() > 0 )
    {
      vector< double > *last = massList.back();
...
double tEquilibCheck::FindLongTermChngRate()
{
  ...
  if( longTime == 0.0 || massList.front() == massList.back() ) longRate = shortRate;
  else
    {
      ca = massList.front();
      na = massList[1];
	  int i = 0; 
      while( na->at(0) < targetTime && !(massList.back()) )
	{
	  ca = na;
	  na = massList[i];
	  i++;
...
tSedTransPwrLawMulti::tSedTransPwrLawMulti( const tInputFile &infile )
{
  ...
  mdGrndiam.assign( miNumgrnsizes,0 );
  mdTauc.assign( miNumgrnsizes,0 );
...
double tSedTransPwrLawMulti::TransCapacity( tLNode *node, int lyr, double weight )
{
  ...
  vector<double> frac; frac.assign( miNumgrnsizes,0 );
  int i;
...
void tErosion::StreamErodeMulti( double dtg, tStreamNet *strmNet, double time )
{
  ...
  vector <double> dz( cn->getNumg(),0 );
  // Depth of deposition/erosion  of each size (erosion = negative)
  vector <double> dzr( cn->getNumg(),0 );
  // Potential depth of bedrock erosion of each grain size
  vector <double> retbr( cn->getNumg(),0 ); //br amt actually ero'd/dep'd
  vector <double> retsed( cn->getNumg(),0  ); //sed amt actually ero'd/dep'd
  vector <double> sedzero( cn->getNumg(),0 );
			   ...
			   tVegetation * /*pVegetation*/ )
{
  ...
  if(strmNet->getRainRate()-strmNet->getInfilt()>0){
    ...
    vector <double> ret( cn->getNumg(),0 ); //amt actually ero'd/dep'd
    vector <double> erolist( cn->getNumg(),0 );
    vector <double> sedzero( cn->getNumg(),0 );
    vector <double> insed( strmNet->getInSedLoadm() );
    vector <double> inletBedSizeFraction( strmNet->getInletSedSizeFraction() );  // TEMP 6/06: stores desired bed sed proportions at inlet
