			       ...
			       tVegetation * /*pVegetation*/ )
{
  ...
  vector<double> valgrd; valgrd.assign(1,0);
...
void tErosion::StreamErodeMulti( double dtg, tStreamNet *strmNet, double time )
{
  ...
  vector <double> dz; dz.assign( cn->getNumg(),0 );
  // Depth of deposition/erosion  of each size (erosion = negative)
  vector <double> dzr; dzr.assign( cn->getNumg(),0 );
  // Potential depth of bedrock erosion of each grain size
  vector <double> retbr; retbr.assign( cn->getNumg(),0 ); //br amt actually ero'd/dep'd
  vector <double> retsed; retsed.assign( cn->getNumg(),0  ); //sed amt actually ero'd/dep'd
  vector <double> sedzero; sedzero.assign( cn->getNumg(),0 );
			   ...
			   tVegetation * /*pVegetation*/ )
{
  ...
  if(strmNet->getRainRate()-strmNet->getInfilt()>0){
    ...
    vector <double> ret; ret.assign( cn->getNumg(),0 ); //amt actually ero'd/dep'd
    vector <double> erolist; erolist.assign( cn->getNumg(),0 );
    vector <double> sedzero; sedzero.assign( cn->getNumg(),0 );
