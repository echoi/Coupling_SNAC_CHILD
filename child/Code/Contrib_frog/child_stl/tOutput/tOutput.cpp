...
void tOutput<tSubNode>::WriteOutput( double time )
{
  ...
  if (!CanonicalNumbering) {
	...
	for (int i = 0; i< size; i++)
      WriteTriangleRecord( titer[i] );
  } else {
    // write triangles in ID order
    deque < tTriangle * > RTri(this->m->getTriList());
    ...
    for(i=1;i<imax-1;i++){
      ...
      tLNode *lnds[3];					// put the nodes in an array
       ...
       if(ct != NULL){				                    // If Triangle present
    	...
    	vector<double> tri_drainage;
		tri_drainage.push_back(lnds[0]->getDrArea());
		tri_drainage.push_back(lnds[1]->getDrArea());
		tri_drainage.push_back(lnds[2]->getDrArea());
...
#undef MY_EXT
  ...
  vector<double> surface; surface.assign(current_ts+1,0);
  vector<double> subsurface; subsurface.assign(current_ts+1,0);		// material in the entire floodplain zone
  vector<double> subsurface_mbelt; subsurface_mbelt.assign(current_ts+1,0);	// material in the narrow meanderbelt zone
  vector<double> depth_age; depth_age.assign(15,0);
