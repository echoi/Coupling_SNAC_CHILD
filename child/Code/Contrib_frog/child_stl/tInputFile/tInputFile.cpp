...
\****************************************************************************/
static
void setKeyWordTable(vector< tKeyPair > &KeyWordTable,
		     tList< tKeyPair > &KeyWordList) {
  KeyWordTable.assign(KeyWordList.getSize(),0);
  int i = 0;
...
void tInputFile::writeLogFile() const
{
  ...
  {
    const int len = KeyWordTable.size();
     for( int i = 0; i < len; ++i ) {
...
vector< tKeyPair > & tInputFile::GetKeyWordTableRef()
{
  return KeyWordTable;
}
