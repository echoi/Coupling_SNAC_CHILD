...
double tBedErodePwrLaw::DetachCapacity( tLNode * n, double dt )
{
  ...
  if(0) //commenting out because gullying inactive in Child6channel
    {
...
#ifdef channel
  return( (n->getHydrWidth()/(n->getFlowEdg())->CalcVEdgLen())*n->getLayerErody(0)*pow(tauex,n->getLayerExpo(0)));
#else
  return( n->getLayerErody(0)*pow(tauex,n->getLayerExpo(0)));
#endif
    }
}
...
#ifdef channel
  erorate = (n->getHydrWidth()/(n->getFlowEdg())->CalcVEdgLen())*n->getLayerErody(0)*pow( erorate, n->getLayerExpo(0) );
#else 
  erorate = n->getLayerErody(0)*pow( erorate, n->getLayerExpo(0) );
#endif  
  n->setDrDt( -erorate );
  return erorate;
}
...
double tBedErodePwrLaw::DetachCapacity( tLNode * n, int i )
{
  ...
  double erorate = tau - n->getTauCrit();
  erorate = (erorate>0.0) ? erorate : 0.0;
  erorate = (n->getHydrWidth()/(n->getFlowEdg())->CalcVEdgLen())*n->getLayerErody(i)*pow( erorate, n->getLayerExpo(0) );
  n->setDrDt( -erorate );
  if(0) //DEBUG
    if(n->getID() == 5){
      std::cout << "kt: " << kt << ", width: " << n->getHydrWidth() << ", mb: " << mb << ", nb: " << nb << ", ns: " << ns << ", vegrough: " << n->getVegRough() << ", layerexpo: " << n->getLayerExpo(0) << std::endl;
      std::cout << "Q: " << n->getQ() << ", slp: " << slp <<std::endl;
      std::cout << "erorate: " << erorate << ", erodibility: " << n->getLayerErody(i) <<std::endl;
      std::cout << "X: " << n->getX() << ", Y: " << n->getY() << ", Z: " << n->getZ() << std::endl;
    }
  ...
  erorate = (erorate>0.0) ? erorate : 0.0;
  erorate = n->getLayerErody(0)*pow( erorate, n->getLayerExpo(0) ); //in this case 0 instead of i, because we assume overland flow, not channelized flow (hf, jul06). This is still eroding only the top most layer. If it's bedrock, then it'll read the erodibility of bedrock.
  n->setDrDt( -erorate );
  if(0) //DEBUG
  if (n->getID() ==100)
...
double tSedTransPwrLaw::TransCapacity( tLNode *node, int lyr, double weight )
{
  ...
  if(0) //commenting out because gullying inactive in Child6channel
    {
      if(slp > 0.84 && atan(slp) > 2* atan((node->getDownstrmNbr())->calcSlope()))
	{
	  std::cout << "do we get in this headcut scenario?? " << std::endl;
	  tPtrList<tLNode > drainingNodes;
	  searchDrainingNodes(node, drainingNodes);
	  tLNode *ndi;
	  tPtrListIter<tLNode> dNIter(drainingNodes);
...
#ifdef channel
      //std::cout << "chanwidth = " << node->getChanWidth() << std::endl;
      //std::cout << "hydrwidth = " << node->getHydrWidth() << std::endl;
      //std::cout << "Q = " << node->getQ() << std::endl;
      tau = kt * pow( node->getQ()/node->getHydrWidth(), mf ) * pow( slp, nf ) * pow ( ns, 1.5 )/ pow( (ns + node->getVegRough()), 0.9 );
#else
      tau = kt*pow( node->getQ()/(node->getFlowEdg())->CalcVEdgLen(), mf ) * pow( slp, nf ) * pow ( ns, 1.5 )/ pow( (ns + node->getVegRough()), 0.9 );
#endif
 ...
 if(0) //DEBUG
    if(node->getID() == 5){
      //std::cout << "kt: " << kt << ", width: " << node->getHydrWidth() << ", mf: " << mf << ", nf: " << nf << ", ns: " << ns << ", vegrough: " << node->getVegRough() <<std::endl;
      //std::cout << "Q: " << node->getQ() << ", slp: " << slp <<std::endl;
      std::cout << "cap: " << cap << ", tauc: " << tauc <<std::endl;
      //std::cout << "X: " << node->getX() << ", Y: " << node->getY() << ", Z: " << node->getZ() << std::endl;
    }
#else
			   ...
			   tVegetation * /*pVegetation*/ )
{
  ...
  if(strmNet->getRainRate()-strmNet->getInfilt()>0){
    ...
    double frac = 0.05;  //fraction of time to zero slope
	...
	double dtgi; //dtg initial (HF, feb 07)
	dtgi = dtg;
    cn = ni.FirstP();
	...
	for( cn = ni.FirstP(); ni.IsActive(); cn = ni.NextP() )
	  {
		...
		if(0) 
		  {
		    double dt;
		    dt = ( cn->getZ() - dn->getZ() ) / ratediff;
		    //std::cout << "dt is " << dt << ", dtmax is: " << dtmax <<  std::endl;
		    if( dt < dtmax ) dtmax = dt;
		    //std::cout << "...so new dtmax is: " << dtmax <<  std::endl;
		  }
		//the next condition is only applied to channels along the "Water way" (HF feb07)
		if( ratediff*dtmax > (cn->getZ() - dn->getZ() ) && (cn->getDrArea()) > (cn->getVArea() * 1) )
		  {
		      
		      std::cout << "got in new loop? "  <<  std::endl;
		      //std::cout << ".. old dtmax is: " << dtmax <<  std::endl;
	    ...
	    dtmax *= frac;  // Take a fraction of time-to-flattening
	      ...
	      }// End for( cn = ni.FirstP()..	    dtmax *= frac;  // Take a fraction of time-to-flattening
	if (dtmax < 0.0000000001) dtmax = 0.0000000001;
	if (dtmax > dtg) dtmax = dtg;
	    timegb+=dtmax;
...
#undef NEWVEG
	...
	if(1) //DEBUG
	  {
	    debugCount++;
	    //std::cout<<" # of cycles "<<debugCount<<" "<< "dtg: " << dtg << " dtmax: " << dtmax << std::endl;
	    if( debugCount > 1e6 )
	      ReportFatalError("More than 1e6 iterations in ErodeDetachLim()" );
	  }
	
	//std::cout<<"Time remaining now "<<dtg<<std::endl;
      } while( dtg>1e-7 );  //Keep going until we've used up the whole time intrvl
...
void tErosion::Diffuse( double rt, bool noDepoFlag , double time)
{
  ...
  double vegTotal; // vegetation cover including dead standing biomass
  ...
  for( ce=edgIter.FirstP(); edgIter.IsActive(); ce=edgIter.NextP() )
    {
      ...
      vegTotal = (cn->getVegCover().getB()+cn->getVegCover().getBd())/(cn->getVegCover().getB()/cn->getVegCover().getVeg()); //calcluation of vegTotal
      if(vegTotal > 1){vegTotal = 1;}
      Kd = kd*exp(-D_alpha*vegTotal); //new Kd(vegTotal) [hf]
      if( 0 ) //DEBUG
	{
  ...
  do
	...
	if (((tLNode *)ce->getOriginPtr())->getNumLayer() < 2)
	...
	else
	  {
	    ...
	    vegTotal = (cn->getVegCover().getB()+cn->getVegCover().getBd())/(cn->getVegCover().getB()/cn->getVegCover().getVeg()); //calcluation of vegTotal
	    if(vegTotal > 1){vegTotal = 1;}
	    Kd = kd*exp(-D_alpha*vegTotal); //new Kd(veg) [hf]
