...
void tNode::ConvertToClosedBoundary()
{
   ...
   tEdge *compe; //now this is its complement (hf 09/2005)
   // Reset boundary flag
   boundary = kClosedBoundary;

   //TODO:  a function that moves the edges to the back of the mesh.edgeList
   // right now this is done in Extricate node, where converttocloseboudnary
   // is called... to make this convert function more general, from here the
   //changes in the list should be made (hf/02/06).
   ...
   do
   {
      ...
      if( ce->getBoundaryFlag()==kNonBoundary )
      {
       	//std::cout << "edge: " << tEdge::EdgeBoundName(ce->FlowAllowed()) << std::endl;
         ce->setFlowAllowed( tEdge::kFlowNotAllowed );
	 //std::cout << "edge after restricting flow: " << tEdge::EdgeBoundName(ce->FlowAllowed())  << std::endl;
	 compe = ce->getComplementEdge();
	 //std::cout << "comp: " << tEdge::EdgeBoundName(compe->FlowAllowed()) << std::endl; 
         compe->setFlowAllowed( tEdge::kFlowNotAllowed );
	 //std::cout << "comp after restrict flow : " << tEdge::EdgeBoundName(compe->FlowAllowed()) << std::endl;
         // get complement and change it too TODO (done, hf 09/2005)
      }
