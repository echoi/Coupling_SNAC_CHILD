		...
		MPI_Comm			communicator )
{
	...
	if ( self->restartTimestep != 0 ) {
		...
		self->restartTimestep=2.4706e+13;
		dtFromFile = 2.29086e07;
	}
	else {
...
void _AbstractContext_Initialise( void* context, void* data ) {
	...
	if(self->rank==0) fprintf(stderr,"In %s: initialization done\n",__func__);
...
void _AbstractContext_Execute( void* context, void* data ) {
	...
	if(self->rank==0) fprintf(stderr,"In %s: starting execution\n",__func__);
	MPI_Barrier(self->communicator);
