...
void _SnacPlSeeds_InitialConditions( void* _context, void* data ) {
	...
	int				index_I,index_J,index_K,global_I_range,global_J_range,global_K_range;
	IJK				ijk;
	...
	printf( "In: %s\n", __func__ );
#endif
    global_I_range = decomp->elementGlobal3DCounts[0];
    global_J_range = decomp->elementGlobal3DCounts[1];
    global_K_range = decomp->elementGlobal3DCounts[2];
    for(index_K = 0; index_K < global_K_range; index_K++) {
    for(index_J = 0; index_J < global_J_range; index_J++) {
        ijk[2] = index_K;
        ijk[0] = (int)(0.5*global_I_range)-(int)(0.5*global_J_range)+index_J;
        ijk[1] = index_J;
